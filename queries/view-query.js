...
function viewRoles(){
    db.query("SELECT role.id, role.title, role.salary, department.dept_name AS department_name FROM role JOIN department ON role.department_id=department.id",(err, result) => {
        if (err) {
          console.log({ error: err.message });
        }
        console.table(result);
